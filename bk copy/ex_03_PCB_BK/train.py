...
if __name__ == "__main__":
    ...
    if config.device == "cuda":
        message = ("Number of GPUs: {n_GPUs}\t" "GPU is_available {available_GPUs}\t" "GPUs name: {name_GPUs}\t" "CUDA version: {version_CUDA}\t" "Current device id: {id_device}").format(
            n_GPUs=torch.cuda.device_count(), available_GPUs=torch.cuda.is_available(), name_GPUs=torch.cuda.get_device_name(), version_CUDA=torch.version.cuda, id_device=torch.cuda.current_device()
        )
        logger.info(message)
    else:
