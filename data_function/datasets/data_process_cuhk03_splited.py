...
class CUHK03_splited(BaseDataset):
    """CUHK03.
    ...
    def __init__(self, root="", split_id=0, cuhk03_labeled=False, cuhk03_classic_split=False, **kwargs):
        ...
        new_gallery_list = []
        for item in train:
            new_train_list.append([osp.join(self.dataset_dir, item[0][31:]), item[1], item[2]])
        for item in query:
            new_query_list.append([osp.join(self.dataset_dir, item[0][31:]), item[1], item[2]])
        for item in gallery:
            new_gallery_list.append([osp.join(self.dataset_dir, item[0][31:]), item[1], item[2]])
        super(CUHK03_splited, self).__init__(new_train_list, new_query_list, new_gallery_list, **kwargs)
