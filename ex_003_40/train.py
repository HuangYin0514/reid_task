...
def brain(config, logger):
    ...
    for epoch in range(config.epochs):
        train_accuracy = utils.avgmeter.AverageMeter()
        ...
        for ind, data in enumerate(tqdm(train_loader)):
            ...
            ### prediction
            optimizer.zero_grad()
            backbone_cls_score, backbone_pool_feats, integrate_cls_score, integrate_mse_loss = model(inputs, pids)
            ...
            #### Integrate loss
            integrate_ce_loss = ce_loss(integrate_cls_score, pids)
            integrate_loss = 0.1 * integrate_ce_loss + 0.007 * integrate_mse_loss

            #### All loss
            loss = backbone_loss + integrate_loss
            ...
            train_accuracy.update(metrics.accuracy(backbone_cls_score, pids)[0].item())
        ...
        if epoch % config.print_every == 0:
            ...
            message = (
                ...
                "Time remaining is {time_H:.0f}h:{time_M:.0f}m"
            ).format(
                epoch + 1,
                config.epochs,
                epoch_loss=epoch_loss,
                accuracy=train_accuracy.avg,
                time_H=time_remaining_H,
                time_M=time_remaining_M,
            )
            logger.info(message)
