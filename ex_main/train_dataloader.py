...
def getData(config):
    ...
    train_loader = torch.utils.data.DataLoader(train_set, sampler=dataloader.samplers.RandomIdentitySampler(dataset.train, config.batch_size, num_instances=4), batch_size=config.batch_size, collate_fn=dataloader.train_collate_fn)
