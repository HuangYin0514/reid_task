...
class FeatsFusionModule(nn.Module):  # AFF
    def __init__(self, config, logger):
        super(FeatsFusionModule, self).__init__()
        self.config = config
        self.logger = logger
    ...
    def forward(self, feats1, feats2):
        ...
        fusion_feats = feats1 * wei + feats2 * (1 - wei)
        return fusion_feats
...
class SuggestFeatsModule(nn.Module):
    def __init__(self, config, logger):
        ...
        self.suggest_layer = nn.Sequential(
            nn.Conv2d(channels, inter_channels, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(inter_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(inter_channels, channels, kernel_size=1, stride=1, padding=0),
...
class ReidNet(nn.Module):
    def __init__(self, num_classes, config, logger, **kwargs):
        ...
        # Feats Fusion Module
        self.featsFusionModule = FeatsFusionModule(config, logger)
    ...
    def forward(self, x):
        ...
        suggest_resnet_feats = self.featsFusionModule(resnet_feats, suggest_feats)
