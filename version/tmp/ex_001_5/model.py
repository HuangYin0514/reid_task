...
class IntegrateFeatsModule(nn.Module):
    def __init__(self, classifier_head, config, logger):
        ...
        conv11 = nn.Conv2d(2048, 1, kernel_size=3, stride=1, padding=1, bias=False)
        bn = nn.BatchNorm2d(1)
    ...
    def forward(self, feats, pids, num_same_id=4):
        ...
        feats_reshaped = weighted_feats.view(chunk_size, num_same_id, 1, h, w)  # 将 feats 重塑为 (chunk_size, 4, 1, h, w)
        integrate_feats = torch.mean(feats_reshaped, dim=1).cuda()  # 计算 integrate_feats，shape 变为 (chunk_size, 1, h, w)
