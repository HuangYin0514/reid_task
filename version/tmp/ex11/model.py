...
class Dynamics(nn.Module):
    def __init__(self, config, logger, **kwargs):
        ...
        self.ode_net4 = ODEBlock(config, logger)
        self.ode_net4.apply(network.utils.weights_init_kaiming)

        self.ode_net5 = ODEBlock(config, logger)
        self.ode_net5.apply(network.utils.weights_init_kaiming)

    def forward(self, x):  # x -> (batch_size, 2048)
        batch_size = x.size(0)
        out = self.ode_net(x)
        # out = self.ode_net2(out)
        # out = self.ode_net3(out)
        # out = self.ode_net4(out)
        # out = self.ode_net5(out)
...
class ReidNet(nn.Module):
    ...
    def forward(self, x):
        ...
        gloab_feat_branch = gloab_feat

        # Dynamics module
        gloab_feat = self.dynamics(gloab_feat)  # (batch_size, feature_dim)

        # Feature extractor module
        gloab_feat = self.feature_extractor(gloab_feat, gloab_feat_branch)  # (batch_size, feature_dim)
