...
class ODEfunc(nn.Module):
    def __init__(self, dim=2048, config=None, logger=None):
        ...
        for _ in range(self.num_conv):
            local_conv = nn.Sequential(conv1x1(dim, dim), norm(dim), nn.ReLU(inplace=True))
            self.conv_list.append(local_conv)
...
class ODEBlock(nn.Module):
    ...
    def __init__(self, config, logger):
        ...
        self.odefunc = ODEfunc(config=config, logger=logger)
