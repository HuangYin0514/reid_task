...
def brain(config, logger):
    ...
    for epoch in range(config.epochs):
        ...
        for ind, data in enumerate(tqdm(train_loader)):
            ...
            gloab_loss = gloab_ce_loss + gloab_tri_loss + 0.0005 * gloab_cent_loss + 0.1 * gloab_ode_loss
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_conv", type=int, default=0, help="")
    parser.add_argument("--outputs_path", type=str, default="", help="")
    args = parser.parse_args()
    config = utils.common.read_config_file(args.config_file)  ## Read the configuration from the provided file
    config.num_conv = args.num_conv  ## Set command-line to config
    outputs_path = args.outputs_path  ## Set command-line to config
    ...
    config.models_outputs_path = models_outputs_path
    config.logs_outputs_path = logs_outputs_path
    config.temps_outputs_path = temps_outputs_path
