...
def brain(config, logger):
    ...
    for epoch in range(config.epochs):
        ...
        for ind, data in enumerate(tqdm(train_loader)):
            ...
            ### prediction
            optimizer.zero_grad()
            G_cls_score, G_pool_feat, G_bn_feat, resnet_feat = model(inputs)

            ### Loss
            #### Gloab loss
            G_ce_loss = ce_labelsmooth_loss(G_cls_score, pids)
            G_tri_loss = triplet_loss(G_pool_feat, pids)
            G_loss = G_ce_loss + G_tri_loss

            #### IP auxiliary loss
            integrat_feats_map, integrat_pids = design_module.IntegratFeatureMap(config).__call__(resnet_feat, pids)
            IP_pool_feat, IP_bn_feat, IP_cls_score = model.auxiliary_classifier_head(integrat_feats_map)
            IP_reasoning_loss = torch.norm(G_bn_feat - IP_bn_feat.repeat_interleave(4, dim=0), p=2)
            IP_ce_loss = ce_labelsmooth_loss(IP_cls_score, integrat_pids)
            IP_auxiliary_loss = IP_ce_loss + 0.007 * IP_reasoning_loss

            #### All loss
            loss = G_loss + IP_auxiliary_loss
