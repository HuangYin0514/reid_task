...
def brain(config, logger):
    ...
    for epoch in range(config.epochs):
        ...
        for ind, data in enumerate(tqdm(train_loader)):
            ...
            #### auxiliary loss
            IP_pool_feat, IP_bn_feat, IP_cls_score = model.auxiliary_classifier_head(resnet_feat)
            IP_reasoning_loss = torch.norm(G_bn_feat - IP_bn_feat)
            IP_ce_loss = ce_labelsmooth_loss(IP_cls_score, pids)
            auxiliary_loss = IP_ce_loss + 0.007 * IP_reasoning_loss
