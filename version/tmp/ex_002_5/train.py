...
def brain(config, logger):
    ...
    for epoch in range(config.epochs):
        ...
        for ind, data in enumerate(tqdm(train_loader)):
            ...
            ### prediction
            optimizer.zero_grad()
            G_cls_score, G_pool_feats, G_bn_feats, resnet_feats, suggest_resnet_feats, suggest_feats = model(inputs)
            ...
            #### Suggest loss
            suggest_loss = mse_loss(suggest_feats, integrate_feats.repeat_interleave(4, dim=0))

            #### All loss
            loss = G_loss + 0.01 * integrate_ce_loss + 0.1 * suggest_loss
            ...
            # print("Loss item", G_ce_loss.item() * inputs.size(0), G_tri_loss.item() * inputs.size(0))
            # print("Auxiliary_loss item", integrate_ce_loss.item() * inputs.size(0), suggest_loss.item() * inputs.size(0))
