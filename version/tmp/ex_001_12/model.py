...
class Integrate_feats_module(nn.Module):
    ...
    def forward(self, feats, pids, num_same_id=4):
        ...
        # CBAM
        CBAM_feats = self._cbam(CAM_feats_reshaped)  #  (bs, 4, h, w)

        # Integrate
        integrate_feats = self.integrate_layer(CBAM_feats)  # (chunk_size, 1, h, w)
        # integrate_feats = torch.mean(CBAM_feats, dim=1, keepdim=True)  # (chunk_size, 1, h, w)
        integrate_pids = pids[::num_same_id]  # 直接从 pids 中获取 integrate_pids

        return integrate_feats, integrate_pids

    def _cbam(self, feats):
        feats = self.ca(feats) * feats
        feats = self.sa(feats) * feats
        return feats
