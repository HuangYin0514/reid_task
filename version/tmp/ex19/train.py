...
def brain(config, logger):
    ...
    for epoch in range(config.epochs):
        ...
        for ind, data in enumerate(tqdm(train_loader)):
            ...
            # quantified_ID_loss = ce_labelsmooth_loss(localized_cls_score, labels)

            localized_bn_feat, localized_cls_score = classifier2(localized_feat_map)
            quantified_ID_loss = ce_labelsmooth_loss(localized_cls_score, labels)

            reasoning_loss = torch.norm(bn_feat - localized_bn_feat, p=2)

            #### All loss
            loss = G_ID_loss + G_tri_loss + quantified_ID_loss + 0.007 * reasoning_loss
