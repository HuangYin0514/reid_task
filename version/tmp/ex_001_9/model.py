...
class IntegrateFeatsModule(nn.Module):
    def __init__(self, classifier_head, config, logger):
        ...
        conv11 = nn.Conv2d(4, 1, kernel_size=1, stride=1, padding=0, bias=False)
        bn = nn.BatchNorm2d(1)
        act = nn.ReLU(inplace=True)
        self.weights_layer = nn.Sequential(conv11, bn)
    ...
    def forward(self, feats, pids, num_same_id=4):
        ...
        feats_reshaped = weighted_feats.view(chunk_size, num_same_id, h, w)  # 将 feats 重塑为 (chunk_size, 4, 1, h, w)
        integrate_feats = 1 * self.weights_layer(feats_reshaped)  # torch.Size([chunk_size, 1, 16, 8])
...
class ReidNet(nn.Module):
    def __init__(self, num_classes, config, logger, **kwargs):
        ...
        self.integrateFeatsModule = IntegrateFeatsModule(self.classifier_head, config, logger)
