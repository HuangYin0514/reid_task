...
def brain(config, logger):
    ...
    classifier2_list = nn.ModuleList([Classifier2(feat_dim=dim, pid_num=num_classes, config=config, logger=logger).to(config.device) for dim in hierarchical_parts_dim])
    ...
    for classifier2 in classifier2_list:
        parameters_to_optimize += list(classifier2.parameters())
    classifier2_params_group = [{"params": parameters_to_optimize, "lr": 0.00035, "weight_decay": 0.0005}]
    ...
    for epoch in range(config.epochs):
        ...
        for ind, data in enumerate(tqdm(train_loader)):
            ...
            ### Prediction
            pool_feat, bn_feat, hierarchical_feat_list = model(inputs)
            ...
            for classifier2_i, hierarchical_feat_i in zip(classifier2_list, hierarchical_feat_list):
                hierarchical_cls_score = classifier2_i(hierarchical_feat_i)
                ce_loss_i = ce_labelsmooth_loss(hierarchical_cls_score, labels)
                tri_loss_i = triplet_loss(hierarchical_feat_i, labels)
                hierarchical_loss += ce_loss_i + tri_loss_i
    
            #### All loss
            loss = G_ID_loss + G_tri_loss + 0.01 * hierarchical_loss
