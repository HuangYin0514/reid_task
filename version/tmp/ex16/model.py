...
class ODEBlock(nn.Module):
    def __init__(self, dim, config, logger, **kwargs):
        ...
        self.odefunc = ODEfunc(dim, config, logger)
...
class Dynamics(nn.Module):
    def __init__(self, dim, config, logger, **kwargs):
        ...
        self.config = config
        self.logger = logger
        self.ode_net = ODEBlock(dim, config, logger)
...
class Hierarchical_Net(nn.Module):
    def __init__(self, c_dim, h_dim, w_dim, config, logger, **kwargs):
        ...
        self.MGM = Multi_Granularity_Module(c_dim, h_dim, w_dim, config, logger)
        self.dynamics = Dynamics(c_dim, config, logger)
...
class Backbone(nn.Module):
    def __init__(self, config, logger, **kwargs):
        ...
        self.hierarchical_net1 = Hierarchical_Net(256, 96, 32, config, logger)
        self.hierarchical_net2 = Hierarchical_Net(512, 48, 16, config, logger)
        self.hierarchical_net3 = Hierarchical_Net(1024, 24, 8, config, logger)
...
class ReidNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Backbone
        resnet_feat, hierarchical_feat1, hierarchical_feat2, hierarchical_feat3 = self.backbone(x)  # (batch_size, 2048, 16, 8)
        hierarchical_feat_list = [hierarchical_feat1, hierarchical_feat2, hierarchical_feat3]
        ...
        if self.training:
            return pool_feat, bn_feat, hierarchical_feat_list
        else:
            return bn_feat
