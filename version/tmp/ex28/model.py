...
class ContrastiveFeats(nn.Module):
    def __init__(self, config, logger):
        super(ContrastiveFeats, self).__init__()
        self.config = config
        self.logger = logger
        ...
        self.contrastive_layer = nn.Sequential(conv11, bn, act)

    def forward(self, feats):
        bs = feats.size(0)
        c = feats.size(1)
        feats = feats.view(bs, c, 1, 1)
        # Contrastive layer
        contrastive_feats = self.contrastive_layer(feats)
        contrastive_feats = contrastive_feats.view(bs, c)
        return contrastive_feats


class IntegrateFeats(nn.Module):
    def __init__(self, config, logger):
        super(IntegrateFeats, self).__init__()
        self.config = config
        self.logger = logger
...
class ReidNet(nn.Module):
    def __init__(self, num_classes, config, logger, **kwargs):
        ...
        # Integrat feats map
        self.integrateFeats = IntegrateFeats(config, logger)

        # Contrastive Feats
        self.contrastiveFeats = ContrastiveFeats(config, logger)
