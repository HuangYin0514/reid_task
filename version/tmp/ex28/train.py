...
def brain(config, logger):
    ...
    for epoch in range(config.epochs):
        ...
        for ind, data in enumerate(tqdm(train_loader)):
            ...
            #### Integrate loss
            integrate_feats, integrate_pids = model.integrateFeats(resnet_feats, pids, num_same_id=4)
            ...
            #### Contrastive loss
            contrastive_feats = model.contrastiveFeats(G_bn_feats)
            contrastive_loss = torch.norm(contrastive_feats - integrate_bn_feats.repeat_interleave(4, dim=0), p=2)
            auxiliary_loss = integrate_ce_loss + 0.1 * contrastive_loss
