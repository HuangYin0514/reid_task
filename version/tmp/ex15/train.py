...
def brain(config, logger):
    ...
    for epoch in range(config.epochs):
        ...
        for ind, data in enumerate(tqdm(train_loader)):
            ...
            ### prediction
            optimizer.zero_grad()
            gloab_score, gloab_feat, hierarchical_score_list, hierarchical_feat_list = model(inputs)
            ...
            for i in range(len(hierarchical_score_list)):
                ce_loss = ce_labelsmooth_loss(hierarchical_score_list[i], labels)
                tri_loss = triplet_loss(hierarchical_feat_list[i], labels)
                hierarchical_loss += ce_loss + tri_loss

            #### All loss
            loss = gloab_loss + 0.5 * hierarchical_loss
