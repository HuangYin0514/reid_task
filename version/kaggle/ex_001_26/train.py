...
def brain(config, logger):
    ...
    for epoch in range(config.epochs):
        ...
        for ind, data in enumerate(tqdm(train_loader)):
            ...
            # specific_loss_1 = torch.einsum("bcd,bcd->b", consistency_features, specific_features).sum() / 15
            # specific_loss_2 = torch.einsum("bcd,bcd->b", specific_features, specific_features).sum() / 15
            ...
            multi_view_loss = consistency_loss + specific_loss_1 + specific_loss_2 + integrate_ce_loss
