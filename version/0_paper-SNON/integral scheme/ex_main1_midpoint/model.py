...
class ODEBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = odeint(self.odefunc, x, integration_time, method="midpoint", rtol=1e-3, atol=1e-3)
        return out[-1]
...
class Robust_Module(nn.Module):
    ...
    def forward(self, x, odefunc):
        ...
        ori_x = torch.abs(x)
        x = ori_x
        x = odeint(odefunc, x, integration_time, method="midpoint_abs", rtol=1e-3, atol=1e-3)
        x = x[-1]
        diff = x - ori_x
