...
def brain(config, logger):
    ...
    center_loss = loss_funciton.CenterLoss(num_classes=num_classes, feature_dim=1024, config=config, logger=logger)
    ...
    for epoch in range(config.epochs):
        ...
        for ind, data in enumerate(tqdm(train_loader)):
            ...
            gloab_loss = gloab_ce_loss + gloab_tri_loss + 0.0005 * gloab_cent_loss + 0.35 * gloab_ode_loss
