...
class ReidNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Backbone
        resnet_feat = self.backbone(x)  # (bs, 1024, 16, 8)

        # Gloab module ([N, 1024])
        gloab_feat = self.gloab_avgpool(resnet_feat)  # (bs, 1024, 1, 1)
        gloab_feat = self.ode_net(gloab_feat)
        gloab_feat = gloab_feat.view(bs, -1)  # (bs, 1024)
        norm_gloab_feat = self.gloab_bottleneck(gloab_feat)  # (bs, 1024)
