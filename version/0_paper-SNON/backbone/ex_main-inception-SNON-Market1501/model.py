...
class ReidNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Backbone
        resnet_feat = self.backbone(x)  # (bs, 1536, 16, 8)

        # Gloab module ([N, 1536])
        gloab_feat = self.gloab_avgpool(resnet_feat)  # (bs, 1536, 1, 1)
        gloab_feat = self.ode_net(gloab_feat)
        gloab_feat = gloab_feat.view(bs, -1)  # (bs, 1536)
        norm_gloab_feat = self.gloab_bottleneck(gloab_feat)  # (bs, 1536)
