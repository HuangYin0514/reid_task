...
from model import Auxiliary_Module, Classifier, ReidNet
...
def brain(config, logger):
    ...
    auxiliary_module = Auxiliary_Module(feat_dim=2048, pid_num=num_classes, config=config, logger=logger).to(config.device)
    ...
    ## auxiliary_module
    auxiliary_module_params_group = [{"params": auxiliary_module.parameters(), "lr": 0.00035, "weight_decay": 0.0005}]
    auxiliary_module_optimizer = torch.optim.Adam(auxiliary_module_params_group)
    auxiliary_module_scheduler = optim.WarmupMultiStepLR(
        auxiliary_module_optimizer,
        milestones=[40, 70],
        gamma=0.1,
        warmup_factor=0.01,
        warmup_iters=10,
    ...
    for epoch in range(config.epochs):
        ...
        for ind, data in enumerate(tqdm(train_loader)):
            ...
            auxiliary_module_optimizer.zero_grad()

            ### Prediction
            G_resnet_feat, G_pool_feat, G_bn_feat = model(inputs)

            ### Loss
            #### Gloab loss
            G_cls_score = classifier(G_bn_feat)
            G_ID_loss = ce_labelsmooth_loss(G_cls_score, labels)
            G_tri_loss = triplet_loss(G_pool_feat, labels)

            #### IP moudule
            feat_map_bn_feat, feat_map_score = auxiliary_module.feat_map_classifier(G_resnet_feat)
            integrat_weighted_features_map, integrating_pids = design_module.IntegratWeightedFeatureMap(config).__call__(
                G_resnet_feat, feat_map_score, labels
            )

            IP_bn_feat, IP_cls_score = auxiliary_module.quantified_classifier(integrat_weighted_features_map)
            IP_ID_loss = ce_labelsmooth_loss(IP_cls_score, integrating_pids)

            reasoning_loss = torch.norm(G_bn_feat - IP_bn_feat.repeat_interleave(4, dim=0), p=2)
            #### All loss
            ...
            auxiliary_module_optimizer.step()
        ...
        auxiliary_module_scheduler.step()
        ...
        if condition1 or condition2:
            ...
            model_path = os.path.join(config.models_outputs_path, "auxiliary_module_{}.tar".format(epoch + 1))
            torch.save(auxiliary_module.state_dict(), model_path)
