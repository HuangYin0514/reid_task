...
class Hierarchical_aggregation(nn.Module):
    def __init__(self, num_classes, config, logger, **kwargs):
        ...
        self.auxiliary_classifier_head_p1 = Auxiliary_classifier_head(256, num_classes, config, logger)
        self.auxiliary_classifier_head_p2 = Auxiliary_classifier_head(512, num_classes, config, logger)
        self.auxiliary_classifier_head_p3 = Auxiliary_classifier_head(1024, num_classes, config, logger)

        self.attention_p1 = ECALayer(256)
        self.attention_p2 = ECALayer(512)
        self.attention_p3 = ECALayer(1024)
    ...
    def forward(
        ...
        p1 = (1 + self.attention_p1(p1)) * p1

        pool_p2 = self.pool_p2(x2)
        p2 = self.cbr_2(pool_p2)
        p2 = (1 + self.attention_p2(p2)) * p2

        pool_p3 = self.pool_p3(x3)
        p3 = self.cbr_3(pool_p3)
        p3 = (1 + self.attention_p3(p3)) * p3

        fc_1_score = self.fc_1(p1)
        fc_2_score = self.fc_2(p2)
        fc_3_score = self.fc_3(p3)

        integrate_feats_p1, integrate_pids = self.integrate_feats_module(p1, pids, fc_1_score, num_same_id=4)
        integrate_cls_score_p1 = self.auxiliary_classifier_head_p1(integrate_feats_p1)

        integrate_feats_p2, integrate_pids = self.integrate_feats_module(p2, pids, fc_2_score, num_same_id=4)
        integrate_cls_score_p2 = self.auxiliary_classifier_head_p2(integrate_feats_p2)

        integrate_feats_p3, integrate_pids = self.integrate_feats_module(p3, pids, fc_3_score, num_same_id=4)
        integrate_cls_score_p3 = self.auxiliary_classifier_head_p3(integrate_feats_p3)

        return fc_1_score, fc_2_score, fc_3_score, integrate_cls_score_p1, integrate_cls_score_p2, integrate_cls_score_p3, integrate_pids
