...
class Integrate_feats_module(nn.Module):
    def __init__(self, config=None, logger=None):
        ...
        self.integrate_block2 = nn.Sequential(
            nn.Conv2d(2048 * 2, 2048, kernel_size=1, stride=1, padding=0, bias=False), nn.BatchNorm2d(2048), nn.ReLU(inplace=True)
        )
    ...
    def forward(self, feats, num_same_id=4):
        ...
        all_feats = torch.cat([feats, integrate_feats], dim=1)
        all_feats = self.integrate_block2(all_feats)
        all_feats_att = self.sigmoid(all_feats)
        integrate_feats = feats + integrate_feats * all_feats_att
