...
class ContrastiveLoss(nn.Module):
    def __init__(self, batch_size, temperature=1, device=None):
...
def brain(config, logger):
    ...
    for epoch in range(config.epochs):
        ...
        for ind, data in enumerate(tqdm(train_loader)):
            ...
            integrate_mse_loss = mse_loss(resnet_feats, integrate_feats.repeat_interleave(4, dim=0))

            #### All loss
            loss = backbone_loss + 0.1 * integrate_ce_loss + 0.01 * integrate_mse_loss
