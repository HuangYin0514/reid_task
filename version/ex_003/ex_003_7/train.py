...
def brain(config, logger):
    ...
    for epoch in range(config.epochs):
        ...
        for ind, data in enumerate(tqdm(train_loader)):
            ...
            fc_1_score, fc_2_score, fc_3_score, integrate_cls_score_p1, integrate_cls_score_p2, integrate_cls_score_p3, integrate_pids = (
                model.hierarchical_aggregation(resnet_feats_x1, resnet_feats_x2, resnet_feats_x3, backbone_cls_score, pids)
            )
            ...
            # integrate
            integrate_ce_loss_p1 = ce_loss(integrate_cls_score_p1, integrate_pids)
            integrate_ce_loss_p2 = ce_loss(integrate_cls_score_p2, integrate_pids)
            integrate_ce_loss_p3 = ce_loss(integrate_cls_score_p3, integrate_pids)
            intergate_loss = integrate_ce_loss_p1 + integrate_ce_loss_p2 + integrate_ce_loss_p3

            #### All loss
            loss = backbone_loss + 0.1 * hierarchical_aggregation_loss + 0.1 * intergate_loss
