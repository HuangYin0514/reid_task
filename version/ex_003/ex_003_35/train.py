...
def brain(config, logger):
    ...
    for epoch in range(config.epochs):
        ...
        for ind, data in enumerate(tqdm(train_loader)):
            ...
            ### prediction
            optimizer.zero_grad()
            backbone_cls_score, backbone_pool_feats, global_cls_score = model(inputs)
            ...
            #### Gloab loss
            gloabl_ce_loss = ce_labelsmooth_loss(global_cls_score, pids)
            gloabl_loss = gloabl_ce_loss

            #### All loss
            loss = backbone_loss + gloabl_loss
