...
def brain(config, logger):
    ...
    for epoch in range(config.epochs):
        ...
        for ind, data in enumerate(tqdm(train_loader)):
            ...
            ### prediction
            optimizer.zero_grad()
            backbone_cls_score, backbone_bn_feats, resnet_feats = model(inputs)
            ...
            integrate_feats = model.integrate_feats_module(resnet_feats, num_same_id=4)
            integrate_bn_feats, integrate_cls_score = model.integrate_classifier_head(integrate_feats)
            integrate_loss = ce_loss(integrate_cls_score, pids)
