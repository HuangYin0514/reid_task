...
class Hierarchical_aggregation(nn.Module):
    def __init__(self, num_classes, config, logger, **kwargs):
        ...
        self.cbr_1_neck = Conv_bn_relu(256, config, logger)
        self.cbr_2_neck = Conv_bn_relu(512, config, logger)
        self.cbr_3_neck = Conv_bn_relu(1024, config, logger)

        self.cbr_1 = Conv_bn_relu(256, config, logger)
        self.cbr_2 = Conv_bn_relu(768, config, logger)
        self.cbr_3 = Conv_bn_relu(1792, config, logger)
    ...
    def forward(
        ...
        x1 = self.cbr_1_neck(x1) + x1
        pool_p1 = self.pool_p1(x1)
        p1 = self.cbr_1(pool_p1)

        x2 = self.cbr_2_neck(x2) + x2
        pool_p2 = self.pool_p2(x2)
        pool_p2 = torch.cat([pool_p2, p1], dim=1)
        p2 = self.cbr_2(pool_p2)

        x3 = self.cbr_3_neck(x3) + x3
        pool_p3 = self.pool_p3(x3)
        pool_p3 = torch.cat([pool_p3, p2], dim=1)
