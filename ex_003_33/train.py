...
def brain(config, logger):
    ...
    for epoch in range(config.epochs):
        ...
        for ind, data in enumerate(tqdm(train_loader)):
            ...
            ### prediction
            optimizer.zero_grad()
            backbone_cls_score, backbone_pool_feats, part_score_list = model(inputs)
            ...
            for score in part_score_list:
                part_ce_loss = ce_loss(score, pids)
                part_loss += 1 / model.num_parts * part_ce_loss

            #### All loss
            loss = backbone_loss + part_loss
