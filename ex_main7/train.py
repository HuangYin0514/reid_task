...
def brain(config, logger):
    ...
    # Loss function
    mse_loss = nn.MSELoss()
    circleLoss = loss_funciton.CircleLoss(m=0.25, gamma=32)
    ...
    for epoch in range(config.epochs):
        ...
        for ind, data in enumerate(tqdm(train_loader)):
            ...
            gloab_circle_loss = circleLoss(*loss_funciton.convert_label_to_similarity(gloab_feat, labels))
            gloab_ode_loss = mse_loss(gloab_feat_steady_diff, torch.zeros_like(gloab_feat_steady_diff, requires_grad=False))

            gloab_loss = gloab_ce_loss + gloab_tri_loss + gloab_circle_loss + 0.1 * gloab_ode_loss
